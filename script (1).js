  ...
  productsUnsub = db.collection("products").onSnapshot(snapshot=>{
    snapshot.forEach(doc=>{
      ...
      if(p){
        ...
        if(typeof data.price === "number") p.price = Math.max(0, data.price);
      }else if(data.name){
  ...
  try{
    ...
    PRODUCTS = DEFAULT_PRODUCTS.map(p => {
      ...
      if(override && typeof override === "object"){
        return { ...p, stock: override.stock !== undefined ? override.stock : p.stock, badge: override.badge || null, price: override.price !== undefined ? override.price : p.price };
      }
...
function saveStock(){
  const map = {};
  PRODUCTS.forEach(p => map[p.id] = { stock:p.stock, badge:p.badge || null, price:p.price });
...
const BADGE_LABELS = { new:"Baru", bestseller:"Terlaris", sale:"Diskon", oursignatures:"Our Signature" };
...
function renderAdminTable(){
    ...
    <tr data-id="${p.id}">
      ...
      <td>
        <div class="price-edit-row">
          <input type="number" min="0" step="500" class="price-edit-input" value="${p.price}" id="priceEdit${p.id}" data-price-id="${p.id}">
          <button class="btn btn-line btn-sm" data-save-price-id="${p.id}">Simpan</button>
        </div>
      </td>
      ...
      <td>
        <select class="badge-select" data-badge-id="${p.id}">
          ...
          <option value="oursignatures" ${currentBadge==="oursignatures"?"selected":""}>Our Signature</option>
        </select>
      </td>
      <td>
  ...
  body.querySelectorAll("[data-save-price-id]").forEach(btn=>{
    btn.onclick = ()=>{
      const id = Number(btn.dataset.savePriceId);
      const input = document.getElementById(`priceEdit${id}`);
      const newPrice = Math.round(Number(input.value));
      if(!Number.isFinite(newPrice) || newPrice < 0){
        toast("Harga tidak valid","err");
        return;
      }
      const p = getProduct(id);
      p.price = newPrice;
      saveStock();
      toast(`Harga "${p.name}" diubah menjadi ${formatRupiah(newPrice)}`,"ok");
      renderGrid();
      renderRecommend();
      if(currentModalProductId === id) renderProductModalBody();
      if(db){
        db.collection("products").doc(String(id)).set({
          ...
          console.error("Gagal sinkron harga ke server:", err);
          toast("Harga tersimpan lokal, gagal sinkron ke server","err");
        });
      }
    };
  });
