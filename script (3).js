...
function closeModalById(id){
  ...
  if(id === "letterModal") resetLetterScene();
  if(!anyModalOpen()) document.body.style.overflow = "";
}
function anyModalOpen(){
  return ["productModal","checkoutModal","confirmModal","loginModal","adminPanel","letterModal"].some(id=>{
...
function bindModalCloseEvents(){
  ...
  document.addEventListener("keydown", (e)=>{
    if(e.key === "Escape"){
      ["productModal","checkoutModal","confirmModal","loginModal","adminPanel","letterModal"].forEach(id=>{
        const el = document.getElementById(id);
...
/* ---------------------------------------------------------
   11b. A LETTER LEFT FOR YOU — Envelope & Letter Experience
--------------------------------------------------------- */
function generateLetterParticles(count){
  const wrap = document.getElementById("letterParticles");
  ...
  for(let i=0;i<count;i++){
    ...
    html += `<span class="letter-particle" style="left:${left}%;width:${size}px;height:${size}px;animation-duration:${duration}s;animation-delay:-${delay}s;"></span>`;
  }
  wrap.innerHTML = html;
}
...
function openLetterModal(){
  ...
  trackEvent("open_letter_section");
  requestAnimationFrame(()=>{
    const closeBtn = document.querySelector("#letterModal .letter-modal-close");
    if(closeBtn) closeBtn.focus();
  });
}

function openLetterInside(){
  ...
  if(envelopeEl) envelopeEl.setAttribute("aria-hidden", "true");
  if(openBtn) openBtn.tabIndex = -1;
  if(closeBtn) closeBtn.tabIndex = 0;
  trackEvent("read_letter");
  setTimeout(()=>{ if(closeBtn) closeBtn.focus(); }, 900);
}

function closeLetterInside(){
  const stage = document.getElementById("letterStage");
  ...
  setTimeout(()=>{
    ...
    const openBtn = document.getElementById("openLetterBtn");
    const closeBtn = document.getElementById("closeLetterBtn");
    if(envelopeEl) envelopeEl.removeAttribute("aria-hidden");
    if(closeBtn) closeBtn.tabIndex = -1;
    if(openBtn){ openBtn.tabIndex = 0; openBtn.focus(); }
  }, 650);
}
...
function resetLetterScene(){
  ...
  if(envelopeEl) envelopeEl.removeAttribute("aria-hidden");
  if(closeBtn) closeBtn.tabIndex = -1;
  if(openBtn) openBtn.tabIndex = 0;
}

function bindLetterExperience(){
  generateLetterParticles(22);
  const footerBtn = document.getElementById("footerEnvelopeBtn");
  const openBtn = document.getElementById("openLetterBtn");
  const closeBtn = document.getElementById("closeLetterBtn");
  if(footerBtn) footerBtn.addEventListener("click", openLetterModal);
  if(openBtn) openBtn.addEventListener("click", openLetterInside);
  if(closeBtn) closeBtn.addEventListener("click", closeLetterInside);
}
